...
let tests = ref([]);
let test = (name, f) => OUnit.(tests := [name >:: f, ...tests^]);
...
let () =
  test("HtmlStream", () => {
    ...
    let (==>) = (s, s') =>
      try({
        let s'' =
          Control.wrapped_output(IO.output_string(), io =>
            HS.parse(IO.nwrite_string(io) $ HS.show_raw', s)
          );
        if (s' == s'') {
          ();
        } else {
    ...
    "< q>" ==> "<q>";
    "<q>" ==> "<q>";
    "<q><b>dsad</b></Q><Br/><a a a>"
    ==> "<q><b>dsad</b></q><br></br><a a='' a=''>";
    "<q x= a=2><q x a=2><q a=2/><q AAaa=2 />"
    ==> "<q x='a=2'><q x='' a='2'><q a='2/'><q aaaa='2'></q>";
    "dAs<b a=\"d'dd\" b='q&q\"qq'></q a=2></><a'a>"
    ==> "dAs<b a='d'dd' b='q&q\"qq'></q></>";
    "dsad<v" ==> "dsad";
    "dsa" ==> "dsa";
    "" ==> "";
    "<" ==> "";
...
let () =
  test("iequal", () =>{
    let t = {
      let n = ref(0);
      x => {
        assert_bool(sprintf("testcase %d", n^), x);
        incr(n);
      };
    };
    let fail = t $ (!);
...
let () =
  test("Stre.before", () =>{
    ...
    t("", Stre.after("abc", "d"));
    let invariant = (s, sub) => {
      let a = Stre.before(s, sub);
      let b = Stre.after(s, sub);
      t(
        ~msg=sprintf("invariant1 %S %S", s, sub),
        s,
        a
        ++ (
          if (String.exists(s, sub)) {
            sub;
          } else {
            "";
          }
      ...
      t(
        ~msg=sprintf("invariant2a %S %S", s, sub),
        a,
        Stre.before(a ++ sub ++ b, sub),
      );
      t(
        ~msg=sprintf("invariant2b %S %S", s, sub),
        b,
        Stre.after(a ++ sub ++ b, sub),
      );
      assert_equal(
        ~msg=sprintf("divide %S %S", s, sub),
        (a, b),
        Stre.divide(s, sub),
      );
    };
...
let () =
  test("Stre.by_words", () =>{
    ...
    let f = (a, l) => t(Stre.split(Stre.by_words, a) == l);
    f("a" ++ String.make(10, '_') ++ "b", ["a", "b"]);
    f("a" ++ String.make(1024, ' ') ++ "b", ["a", "b"]);
    f("a" ++ String.make(10240, ' ') ++ "b", ["a", "b"]);
    f(
      "проверка unicode!  www.ahrefs.com:работает ",
      ["проверка", "unicode", "www.ahrefs.com", "работает"],
    );
    let make = (sep, n, s) => String.concat(sep) @@ List.make(n, s);
    f(make(" ", 10) @@ make("", 10240, "�"), []);
  });
  ...
  test("Network.string_of_ipv4", () =>{
    let t = (n, s) => {
      assert_equal(
        ...
        n,
        Network.int32_of_ipv4 @@ Network.ipv4_of_string_null(s),
      );
      assert_equal(
        ~printer=id,
        Network.string_of_ipv4 @@ Network.ipv4_of_int32(n),
        s,
      );
    };
      ...
      try(
        assert_equal(
          ...
          Network.ipv4_matches(
            Network.ipv4_of_string_null(ip),
            Network.cidr_of_string_exn(mask),
          ),
        )
      ) {
...
let () =
  test("Time.basic_string", () =>{
    ...
    t(
      U.timegm @@ U.strptime("%Y%m%d %H%M%S", "20200101 123456"),
      "2020-01-01 12:34:56",
    );
    t(
      U.timegm @@ U.strptime("%Y%m%d %H%M%S", "20190101 123456"),
      "2019-01-01 12:34:56",
    );
    t(
      U.timegm @@ U.strptime("%Y%m%d %H%M%S", "20090101 123456"),
      "2009-01-01 12:34:56",
    );
  });

// let () =
//   test("Time.compact_duration", () =>
//     let t = (n, s) =>
...
//     let fail = s =>
//       switch (Time.of_compact_duration(s)) {
//       | exception _ => ()
//       | _ => assert_failure(s)
//       };
...
// let () =
//   test("Action.stable_partition", () =>
//     let t = (l, n, result) =>
//       assert_equal(
//         ~msg=sprintf("stable_partition %d", n),
//         ~printer=strl(strl(string_of_int)),
//         result,
//         Action.stable_partition(n, l),
//       );
...
//       assert_equal(
//         ~msg=sprintf("stable_partition %d", n),
//         ~printer=strl(string_of_int),
//         l,
//         Action.(stable_unpartition @@ stable_partition(n, l)),
//       );
//       assert_equal(
//         ~msg=sprintf("distribute %d", n),
//         ~printer=strl(string_of_int),
//         l,
//         Action.(undistribute @@ distribute(n, l)),
//       );
//     };
...
//   test("Action.chunk", () =>
//     let t = (l, n, result) => {
//       let printer = strl(strl(string_of_int));
//       assert_equal(
//         ~msg=sprintf("chunk %d", n),
//         ~printer,
//         result,
//         List.map(List.rev) @@ Action.chunk(n, l),
//       );
//       assert_equal(
//         ~msg=sprintf("chunk_a %d", n),
//         ~printer,
//         result,
//         List.map(Array.to_list) @@ Action.chunk_a(n) @@ Array.of_list(l),
//       );
//     };
...
// let () =
//   test("Enum.align", () =>
//     let e1 = List.enum([1, 3, 6]);
//     let e2 = List.enum([2, 4, 5, 7, 8]);
//     let l = List.of_enum @@ Enum.align(compare, e1, e2);
//     let expect = [1, 2, 3, 4, 5, 6, 7, 8];
//     OUnit.assert_equal(~printer=strl(string_of_int), expect, l);
//   );

// let () =
//   test("Enum.group_assoc", () => {
//     OUnit.assert_equal(
//       ~msg="1",
//       [("ds", 3), ("dsa", 7), ("ds", 11)],
//       List.of_enum @@
...
//         strl(
//           uncurry @@
//           (sprintf("%s, %s") $$ Option.map_default(string_of_int, "NONE")),
//         );
...
//         strl(
//           fun
//           | `Left(x) => sprintf("L %d", x)
//           | `Right(x) => sprintf("R %d", x)
//           | `Both(x, y) => sprintf("(%d,%d)", x, y),
//         );
...
//             expect,
//           );
//         join(Factor.Int.compare, id, List.enum(e1), List.enum(e2))
//         |> List.of_enum
//         |> OUnit.assert_equal(~printer, expect);
//       };

//       t(
//         Enum.join_full_multi_by_key,
//         [1, 2, 3, 4],
//         [0, 2, 4, 8],
//         [
//           (None, Some(0)),
...
//       [1, 20, 1, 2, 3, 44],
//       List.of_enum @@
//       Enum.uniq((x, y) => x mod 10 == y mod 10) @@
//       List.enum([1, 11, 20, 100, 0, 1, 2, 3, 133, 44]),
//     );
//     OUnit.assert_equal(
//       ~msg="4",
//       ~printer=Std.dump,
...
//       List.of_enum @@
//       Enum.count_unique((==)) @@
//       List.enum([1, 2, 2, 3, 1, 1, 1, 1, 4]),
//     );
//   });

// let () =
//   test("Enum.take", () =>
//     let e = Enum.take(3, Enum.empty());
//     OUnit.assert_bool("is_empty", Enum.is_empty(e));
//     OUnit.assert_bool("peek None", Enum.peek(e) == None);
//     let e = Enum.take(3, List.enum @@ List.init(10, id));
//     OUnit.assert_equal(
//       ~printer=strl(string_of_int),
//       [0, 1, 2],
//       List.of_enum(e),
...
// let () =
//   test("Enum.iter_while", () =>
//     let e = List.enum([1, 2, 3, 4, 5, 6, 7]);
//     Enum.iter_while(
//       x => {
//         if (x == 2) {
//           Enum.iter_while(x => x < 6, e);
//         };
//         x < 4;
//       },
//       e,
...
//       assert_equal(L.lfu_free(cache), 0);
//       assert_bool("key 1 evicted", (!) @@ L.mem(cache, 1));
//     );
...
//       assert_equal(L.lru_free(cache), 2);
//       assert_equal(L.lfu_free(cache), 2);
//       assert_bool("key 1 removed", (!) @@ L.mem(cache, 1));
//       L.put(cache, 2, "b");
//       L.put(cache, 3, "c");
//       let _ = L.get(cache, 2);
//       L.put(cache, 4, "d");
...
//       assert_equal(L.lfu_free(cache), 2);
//       assert_equal(L.lru_free(cache), 0);
//       assert_bool("key 2 removed", (!) @@ L.mem(cache, 2));
...
// let () =
//   test("Timer.start", () =>
//     let timer = (new Action.timer_start)(0.5);
...
//     assert_bool(
//       "timer mark2 not 500ms",
//       timer#json == [("mark", `Int(0)), ("mark2", `Int(500))],
//     );
//     timer#reset;
...
//   (
//     () => {
//       module I = Idn.Make(CONV_Netconversion);
//       I.self_test();
//     }
//   );

// module Bit_list_test =
//   Bit_struct_list.Make({
//     let item_bits = 3;
//     let pp = string_of_int;
//   });

// let () =
//   test("bit_struct_list") @@
//   (
//     () => {
//       let printer = l => strl(string_of_int, l);
//       open Bit_list_test;
//       let t = l =>
//         OUnit.assert_equal(
//           ~printer,
//           l,
//           to_list @@ inject @@ project @@ of_list(l),
//         );
...
// let tests = () => {
//   let _: test_results = run_test_tt_main("devkit" >::: List.rev(tests^));
//   ();
// };
