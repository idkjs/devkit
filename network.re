...
/* reexport exception */
include (Devkit_ragel: {exception Parse_ipv4(string);});
...
let bytes_of_ipv4 = addr => {
  let a =
    Int32.to_int @@
    Int32.shift_right_logical(Int32.logand(0xFF000000l, addr), 24);
  let b =
    Int32.to_int @@
    Int32.shift_right_logical(Int32.logand(0x00FF0000l, addr), 16);
  let c =
    Int32.to_int @@
    Int32.shift_right_logical(Int32.logand(0x0000FF00l, addr), 8);
  let d = Int32.to_int @@ Int32.logand(0x000000FFl, addr);
  (a, b, c, d);
};

let string_of_ipv4 = addr => {
  let (a, b, c, d) = bytes_of_ipv4(addr);
...
let ipv4_of_string_null = s =>
  try(ipv4_of_string_exn(s)) {
  | _ => 0l
  };
/*
...
let make_broadcast = (addr, netmask) =>
  Int32.logor(addr, Int32.lognot(netmask));
...
let cidr_of_string_exn = s =>
  Scanf.sscanf(
    ...
    (ip, len) => {
      if (len < 0 || len > 32) {
        Exn.fail("bad cidr %s", s);
      };
      let mask =
        if (len == 0) {
          0l;
        } else {
          Int32.lognot @@ Int32.pred @@ Int32.shift_left(1l, 32 - len);
        };
      let ip = ipv4_of_string_exn(ip);
      (Int32.logand(ip, mask), mask);
    },
  );

let cidr_of_string_exn = s =>
  try((ipv4_of_string_exn(s), (-1l))) {
  | Parse_ipv4(_) => cidr_of_string_exn(s)
  };
...
let public_ipv4_network_ips = () =>
  U.getifaddrs()
  |> List.filter(((_, ip)) =>
       switch (ipv4_of_string_exn(ip)) {
       | exception _ => false
       | ip => (!) @@ is_ipv4_special(ip)
       }
     )
  |> List.map(((iface, ip)) => (iface, Unix.inet_addr_of_string(ip)));
...
let private_ipv4_network_ip = () =>
  switch (private_ipv4_network_ips()) {
  | [] => Unix.inet_addr_loopback
  | [(_, ip), ..._] => ip
  };

let public_ipv4_network_ip = () =>
  switch (public_ipv4_network_ips()) {
  | [] => None
  | [(_, ip), ..._] => Some(ip)
  };
let public_ipv4_network_ip_exn = () =>
  switch (public_ipv4_network_ip()) {
  | None => Exn.fail("Network: no public ipv4 address")
  | Some(ip) => ip
  };
...
let ipv4_of_yojson = j =>
  switch (j) {
  | `String(i) =>
    try(Ok(ipv4_of_string_exn(i))) {
    | exn =>
      Error(
        Printf.sprintf("ipv4: cannot parse %S (%s)", i, Exn.to_string(exn)),
      )
    }
